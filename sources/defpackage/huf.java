package defpackage;

import android.os.CancellationSignal;
import kotlin.coroutines.Continuation;

/* renamed from: huf  reason: default package */
public final class huf extends ffe implements a66 {
    public int X;
    public final /* synthetic */ ouf Y;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    public huf(ouf ouf, Continuation continuation) {
        super(2, continuation);
        this.Y = ouf;
    }

    public final Object invoke(Object obj, Object obj2) {
        return ((huf) n((sx3) obj, (Continuation) obj2)).o(e5f.a);
    }

    public final Continuation n(Object obj, Continuation continuation) {
        return new huf(this.Y, continuation);
    }

    public final Object o(Object obj) {
        tx3 tx3 = tx3.a;
        int i = this.X;
        if (i == 0) {
            od2.a0(obj);
            ouf ouf = this.Y;
            mtf a = ouf.a(ouf);
            this.X = 1;
            a.getClass();
            xlc a2 = xlc.a(2, "SELECT token FROM webapp_biometry WHERE user_id = ? AND bot_id = ?");
            a2.j(1, ouf.a);
            a2.j(2, ouf.b);
            obj = ote.i(a.a, new CancellationSignal(), new jtf(a, a2, 2), this);
            if (obj == tx3) {
                return tx3;
            }
        } else if (i == 1) {
            od2.a0(obj);
        } else {
            throw new IllegalStateException("call to 'resume' before 'invoke' with coroutine");
        }
        return obj;
    }
}

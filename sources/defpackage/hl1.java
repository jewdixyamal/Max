package defpackage;

import android.content.Context;
import android.content.Intent;
import android.os.Handler;
import android.os.Looper;
import java.util.ArrayList;
import java.util.Iterator;
import one.me.calls.impl.service.CallServiceImpl;

/* renamed from: hl1  reason: default package */
public final class hl1 {
    public static final String a(int i) {
        hl1 hl1 = CallServiceImpl.s0;
        if (i == -1) {
            return "manifest";
        }
        ArrayList arrayList = new ArrayList();
        Iterator it = CallServiceImpl.u0.iterator();
        while (((jy7) it).hasNext()) {
            int intValue = ((Number) ((hy7) it).next()).intValue();
            if (!(intValue == -1 || (i & intValue) == 0)) {
                arrayList.add(intValue != -1 ? intValue != 2 ? intValue != 32 ? intValue != 64 ? intValue != 128 ? wg0.g(intValue, "unknown(", ")") : "microphone" : "camera" : "mediaProjection" : "mediaPlayback" : "manifest");
            }
        }
        return x53.n0(arrayList, "|", (String) null, (String) null, (m56) null, 62);
    }

    public static void b(Context context, Intent intent) {
        try {
            context.startService(intent);
        } catch (Throwable th) {
            String message = th.getMessage();
            hm9.p("CallServiceTag", "cant start service... handle exception: " + message, th);
            c().post(new c(context, 16, intent));
        }
    }

    public static Handler c() {
        if (CallServiceImpl.t0 == null) {
            CallServiceImpl.t0 = new Handler(Looper.getMainLooper());
        }
        Handler handler = CallServiceImpl.t0;
        if (handler != null) {
            return handler;
        }
        throw new IllegalArgumentException("Required value was null.".toString());
    }

    public static void d(Context context) {
        Intent intent = new Intent(context, CallServiceImpl.class);
        gl1 gl1 = gl1.a;
        b(context, intent.putExtra("ACTION", 3));
    }
}

...
public abstract class mpf {
    ...
    public static final kpf b(ViewGroup viewGroup, k56 k56) {
        ViewTreeObserver viewTreeObserver = viewGroup.getViewTreeObserver();
        lpf lpf = new lpf(k56, viewTreeObserver, viewGroup);
        viewTreeObserver.addOnGlobalLayoutListener(lpf);
        if (viewGroup.isAttachedToWindow()) {
            if (!viewGroup.isAttachedToWindow()) {
                a(lpf, viewTreeObserver, viewGroup);
            } else {
                viewGroup.addOnAttachStateChangeListener(new jpf(viewGroup, lpf, viewTreeObserver, viewGroup, 1));
            }
        } else if (!viewGroup.isAttachedToWindow()) {
            viewGroup.addOnAttachStateChangeListener(new jpf(viewGroup, lpf, viewTreeObserver, viewGroup, 0));
        } else if (!viewGroup.isAttachedToWindow()) {
            a(lpf, viewTreeObserver, viewGroup);
        } else {
            viewGroup.addOnAttachStateChangeListener(new jpf(viewGroup, lpf, viewTreeObserver, viewGroup, 2));
        }
        return new kpf(viewTreeObserver, viewGroup, lpf);
    }

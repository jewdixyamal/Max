package defpackage;

/* renamed from: efc  reason: default package */
public final class efc extends jw6 {
    public static final efc u0 = new efc();
    public final transient Object X;
    public final transient Object[] Y;
    public final transient int Z;
    public final transient int s0;
    public final transient efc t0;

    public efc() {
        this.X = null;
        this.Y = new Object[0];
        this.Z = 0;
        this.s0 = 0;
        this.t0 = this;
    }

    public final jx6 c() {
        return new hfc(this, this.Y, this.Z, this.s0);
    }

    public final jx6 d() {
        return new ifc(this, new jfc(this.Z, this.s0, this.Y));
    }

    public final boolean g() {
        return false;
    }

    public final Object get(Object obj) {
        Object k = kfc.k(this.X, this.Y, this.s0, this.Z, obj);
        if (k == null) {
            return null;
        }
        return k;
    }

    public final int size() {
        return this.s0;
    }

    public efc(int i, Object[] objArr) {
        this.Y = objArr;
        this.s0 = i;
        this.Z = 0;
        int h = i >= 2 ? jx6.h(i) : 0;
        Object j = kfc.j(objArr, i, h, 0);
        if (!(j instanceof Object[])) {
            this.X = j;
            Object j2 = kfc.j(objArr, i, h, 1);
            if (!(j2 instanceof Object[])) {
                this.t0 = new efc(j2, objArr, i, this);
                return;
            }
            throw ((bx6) ((Object[]) j2)[2]).a();
        }
        throw ((bx6) ((Object[]) j)[2]).a();
    }

    public efc(Object obj, Object[] objArr, int i, efc efc) {
        this.X = obj;
        this.Y = objArr;
        this.Z = 1;
        this.s0 = i;
        this.t0 = efc;
    }
}

...
public final class FileCacheWriter {
    ...
    private void atomicWrite(boolean z, CallAnalyticsEvent callAnalyticsEvent, OutputStream outputStream) throws IOException {
        ...
        try {
            ...
            gZIPOutputStream = this.compressContent ? new GZIPOutputStream(this.proxyStream, LockFreeTaskQueueCore.MIN_ADD_SPIN_CAPACITY, true) : this.proxyStream;
    ...
    private void write(boolean z, CallAnalyticsEvent callAnalyticsEvent, OutputStream outputStream) throws IOException {
        ...
        p2b p2b = new p2b(new xna(outputStream));
        try {
            EventSerializer.INSTANCE.serialize(p2b, callAnalyticsEvent);
            p2b.close();
            return;
        } catch (Throwable th) {
            th.addSuppressed(th);

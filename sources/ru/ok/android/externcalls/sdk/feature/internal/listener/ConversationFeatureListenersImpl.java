...
public final class ConversationFeatureListenersImpl implements ConversationFeatureListeners {
    ...
    private final Map<u61, FeatureRoles> getFeaturesRolesMap(y61 y61) {
        LinkedHashMap linkedHashMap = new LinkedHashMap();
        for (u61 u61 : u61.values()) {
            Set set = (Set) y61.a.get(u61);
            if (set == null || set.isEmpty()) {
                linkedHashMap.put(u61, FeatureRoles.EnabledForAll.INSTANCE);
            } else {
                linkedHashMap.put(u61, new FeatureRoles.EnabledForRoles(set));
            }
        }
        return linkedHashMap;
    }

    private final void notifyListenersWithNewEnabledFeatures(Set<? extends u61> set, Set<? extends u61> set2) {
        for (u61 u61 : bcd.Y(set2, set)) {
            Set<ConversationFeatureManager.FeatureListener> set3 = this.listenersMap.get(u61);
            if (set3 != null) {
                for (ConversationFeatureManager.FeatureListener onFeatureEnabledChanged : set3) {
                    onFeatureEnabledChanged.onFeatureEnabledChanged(u61, false);
                }
            }
        }
        for (u61 u612 : bcd.Y(set, set2)) {
            Set<ConversationFeatureManager.FeatureListener> set4 = this.listenersMap.get(u612);
            if (set4 != null) {
                for (ConversationFeatureManager.FeatureListener onFeatureEnabledChanged2 : set4) {
                    onFeatureEnabledChanged2.onFeatureEnabledChanged(u612, true);
                }
            }
        }
    }

    private final void notifyListenersWithNewFeaturesRoles(Map<u61, ? extends FeatureRoles> map, Map<u61, ? extends FeatureRoles> map2) {
        Set<ConversationFeatureManager.FeatureListener> set;
        for (u61 u61 : bcd.Z(map.keySet(), map2.keySet())) {
            if (!tpa.f(map.get(u61), map2.get(u61)) && (set = this.listenersMap.get(u61)) != null) {
                for (ConversationFeatureManager.FeatureListener featureListener : set) {
                    ...
                    Object obj = map2.get(u61);
                    if (obj == null) {
                        obj = companion.createDisabledForAll();
                    }
                    featureListener.onFeatureRolesChanged(u61, (FeatureRoles) obj);
                }
            }
        }
    }

    private final void notifyNewListener(u61 u61, ConversationFeatureManager.FeatureListener featureListener) {
        featureListener.onFeatureEnabledChanged(u61, isFeatureEnabled(u61));
        featureListener.onFeatureRolesChanged(u61, getFeatureRoles(u61));
    }

    public void addFeatureListener(u61 u61, ConversationFeatureManager.FeatureListener featureListener) {
        EnumMap<u61, Set<ConversationFeatureManager.FeatureListener>> enumMap = this.listenersMap;
        Set<ConversationFeatureManager.FeatureListener> set = enumMap.get(u61);
        if (set == null) {
            set = new HashSet<>();
            enumMap.put(u61, set);
        }
        set.add(featureListener);
        notifyNewListener(u61, featureListener);
    }

    public FeatureRoles getFeatureRoles(u61 u61) {
        Map<u61, ? extends FeatureRoles> map = this.featureRolesMap;
        FeatureRoles.Companion companion = FeatureRoles.Companion;
        Object obj = map.get(u61);
        if (obj == null) {
            obj = companion.createDisabledForAll();
        }
    ...
    public boolean isFeatureEnabled(u61 u61) {
        return this.enabledFeatureSet.contains(u61);
    }

    public final void onFeatureSetChanged(x61 x61) {
        Set<? extends u61> set = this.enabledFeatureSet;
        Set<? extends u61> set2 = x61.a;
        notifyListenersWithNewEnabledFeatures(set, set2);
        this.enabledFeatureSet = set2;
    }

    public final void onFeaturesPerRoleChanged(y61 y61) {
        Map<u61, FeatureRoles> featuresRolesMap = getFeaturesRolesMap(y61);
        notifyListenersWithNewFeaturesRoles(this.featureRolesMap, featuresRolesMap);
        this.featureRolesMap = featuresRolesMap;
    }

    public void removeFeatureListener(u61 u61, ConversationFeatureManager.FeatureListener featureListener) {
        Set set = this.listenersMap.get(u61);
        if (set != null) {

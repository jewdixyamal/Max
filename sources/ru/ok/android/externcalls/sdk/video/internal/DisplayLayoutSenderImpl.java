...
public final class DisplayLayoutSenderImpl implements DisplayLayoutSender {
    ...
    public DisplayLayoutSenderImpl(m56 m56, m56 m562) {
        this.store = m56;
        this.sender = m562;
    }
    ...
    public void sendDisplayLayouts(Collection<ConversationDisplayLayoutItem> collection) {
        ...
        if (sendFilter == null || sendFilter.shouldSend()) {
            ...
            for (ConversationDisplayLayoutItem next : collection) {
                olf type = next.getVideoTrackParticipantKey().getType();
                ...
                if (!(conversationParticipant == null || !conversationParticipant.isUseable() || (bg1 = conversationParticipant.getCallParticipant().a) == null)) {
                    adb adb = new adb(3);
                    adb.a = bg1;
                    adb.b = type;
                    adb.c = next.getVideoTrackParticipantKey().getMovieId();
                    arrayList.add(new u41(adb.c(), next.getLayout()));
                }
            }

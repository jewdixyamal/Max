...
public final class VideoRenderManagerImpl implements VideoRenderManager, pg4 {
    ...
    public VideoRenderManagerImpl(k56 k56, py0 py0, ConversationRenderers conversationRenderers2, ParticipantStore participantStore) {
        this.state = k56;
        this.call = py0;
    ...
    private final void setRenderersForMe(fp1 fp1, List<? extends VideoSink> list) {
        int ordinal = fp1.a.ordinal();
        if (ordinal == 0) {
            py0 py0 = this.call;
    ...
    private final void setRenderersForOthers(fp1 fp1, List<? extends VideoSink> list) {
        setRenderers(fp1, list);
        py0 py0 = this.call;
        if (py0.i()) {
            py0.t1.R(fp1, list);
    ...
    public Map<fp1, List<VideoSink>> getRemoteVideoRenderers(bg1 bg1) {
        Map<fp1, List<VideoSink>> map = this.cache.get(bg1);
    ...
    public void setRenderers(ConversationVideoTrackParticipantKey conversationVideoTrackParticipantKey, List<? extends VideoSink> list) {
        ...
        if (conversationParticipant != null && conversationParticipant.isUseable()) {
            adb adb = new adb(3);
            adb.b = conversationVideoTrackParticipantKey.getType();
            adb.a = conversationParticipant.getCallParticipant().a;
            adb.c = conversationVideoTrackParticipantKey.getMovieId();
            fp1 c = adb.c();
            ...
            if (conversationParticipant == this.store.getMe()) {
                setRenderersForMe(c, list);
            } else {
                setRenderersForOthers(c, list);
            }
        }
    }

    private final void setRenderers(fp1 fp1, List<? extends VideoSink> list) {
        HashMap<bg1, Map<fp1, List<VideoSink>>> hashMap = this.cache;
        bg1 bg1 = fp1.b;
        Map<fp1, List<VideoSink>> map = hashMap.get(bg1);
        if (map == null) {
            map = new HashMap<>();
            hashMap.put(bg1, map);
        }
